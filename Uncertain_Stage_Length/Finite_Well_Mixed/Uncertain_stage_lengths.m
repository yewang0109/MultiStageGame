...
parfor times=1:6
    ...
    for cont = 1:500   % Repeat simulations
        ...
        while allRowsEqual(sta) ~= 1
            Pay = DG_Uncertain_WellMixed_multiple(beta, delta,b, c, N, sta,stra_matrix,MS,L); % Calculate payoffs
            sta = DB_WellMixed_multiple(Pay, N, sta, w); % Update strategies
            tt=tt+1;
        end
        pre_stra = zeros(T-tt, MS); % Record strategies at homogeneous states
            ...
            if allRowsEqual(sta) == 1
                ...
                if p < mu     % Mutation condition
                    ...
                    if ~isequal(sta(i, :), sta1)
                        sta2 = sta(i, :);
                        numbersta1 = N - 1;
                        Pay = DG_Uncertain_WellMixed(beta, delta,b, c, N, sta, sta1, sta2, numbersta1); % Calculate payoffs
                        sta = DB_WellMixed(Pay, N, sta, w, sta1, sta2, numbersta1); % Update strategies
                    end
                end
            else
                pre_stra(t, :) = zeros(1, MS);
                Pay = DG_Uncertain_WellMixed(beta, delta,b, c, N, sta, sta1, sta2, numbersta1);
...
pro_uncertain_L_1_positive=proportion
