...
parfor times=1:6
    ...
    for cont = 1:500       % Repeat simulations
        ...
        while allRowsEqual(sta) ~= 1
            Pay = DG_Transition_WellMixed_Multiple_New(N, sta,stra_matrix,MS,L,repeated_pay); % Calculate payoffs
            ...
            if allRowsEqual(sta) == 1
                ...
                if p < mu     % Mutation condition
                    ...
                    if ~isequal(sta(i, :), sta1)
                        sta2 = sta(i, :);
                        numbersta1 = N - 1;
                        Pay = DG_Transition_WellMixed_New(N, sta, sta1, sta2, repeated_pay,stra_matrix,numbersta1); % Calculate payoffs
                        sta = DB_WellMixed(Pay, N, sta, w, sta1, sta2, numbersta1); % Update strategies
                    end
                end
            else
                pre_stra(t) = 0;
                Pay = DG_Transition_WellMixed_New(N, sta, sta1, sta2, repeated_pay, stra_matrix, numbersta1);
...
Game_transition_mixed=proportion;