...
parfor times=1:6
    ...
    for cont = 1:500    % Repeat simulations
        ...
        while allRowsEqual(sta) ~= 1
            Pay = DG_Dependent_WellMixed_Multiple_New(N, sta,stra_matrix,MS,L,repeated_pay); % Calculate payoffs
        ...
        for t = 1:T-tt             % Iterate through time steps
            ...
            if allRowsEqual(sta) == 1
                ...
                pre_stra(t) = 0;
                Pay = DG_Dependent_WellMixed_New(N, sta, sta1, sta2, repeated_pay, stra_matrix, numbersta1);
