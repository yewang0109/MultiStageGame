...
proportion=zeros(1,10); % Initialize cooperation proportion array
...
parfor delta_cont=1:10
    delta=(delta_cont-1)/10;
    repeated_pay=zeros((L-1)^MS,(L-1)^MS);
    for j=1:(L-1)^MS
        sta1=stra_matrix(j,:);
        for jj=j:(L-1)^MS
            sta2=stra_matrix(jj,:);
            expected_pay=payoff_game_transition(sta1, sta2, b, c, beta, delta, 5000,10000);
    ...
    for cont = 1:500     % Repeat simulations
        ...
        while allRowsEqual(sta) ~= 1
            Pay = DG_Transition_Structured_Multiple_New(Gr, N, sta,repeated_pay,stra_matrix); % Calculate payoffs
            ...
            if allRowsEqual(sta) == 1
                ...
                if p < mu     % Mutation condition
                    ...
                    if ~isequal(sta(i, :), sta1)
                        sta2 = sta(i, :);
                        numbersta1 = N - 1;
                        Pay = DG_Transition_Structured_New(Gr, N, sta, sta1, sta2, repeated_pay,stra_matrix); % Calculate payoffs
                        sta = DB_Structured(Pay, Gr, N, sta, w); % Update strategies
                    end
                end
            else
                pre_stra(t) = 0;
                Pay = DG_Transition_Structured_New(Gr, N, sta, sta1, sta2, repeated_pay,stra_matrix);
    ...
    proportion(delta_cont) = coop / cont;
end
Transition_regular_delta=proportion;